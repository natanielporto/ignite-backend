...
class RentalDevolutionUseCase {
  ...
  async execute({ id, user_id }: IRequest): Promise<Rental> {
    ...
    let daysToCharge = this.dateProvider.compareInDays(
      rental.start_date,
      this.dateProvider.dateNow()
    );

    if (daysToCharge <= 0) daysToCharge = minimumDaysToCharge;
    ...
    total += daysToCharge * car.daily_rate;
